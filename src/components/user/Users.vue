<template>
  <div>
      ...
      <el-table :data="userList" border style="width: 100%; margin: 10px 0">
        ...
        <el-table-column prop="mg_state" label="状态">
          <template #default="props">
            <el-switch @change="changeStatus(props.row.id, $event)" v-model="props.row.mg_state"></el-switch>
          </template>
        </el-table-column>
        <el-table-column prop="address" label="操作">
          <template #default="props">
            ...
            <el-tooltip class="item" effect="dark" content="分配角色" placement="top-start">
              <el-button type="warning" size="mini" @click="showRoles(props.row)">s</el-button>
            </el-tooltip>
          </template>
        </el-table-column>/
      </el-table>
    ...
    <!-- 添加用户弹出输入框 -->
    <el-dialog title="添加用户" v-model="addUserVisible" width="50%" @close="closeAddUserDialog">
      <el-form
        ref="addUserFormRef"
        :model="addUserForm"
    ...
    <!-- 修改用户信息对话框 -->
    <el-dialog title="修改信息" v-model="editUserVisible" width="50%">
      <el-form
    ...
    <el-dialog title="修改用户" v-model="rolesDialogVisible" width="50%">
      <!--主体部分 -->
      <div>
        <p>
          当前的用户:
        ...
        <span>分配新角色:</span>
        <el-select v-model="rolesId" slot="prepend" placeholder="请选择" style="margin-bottom: 10px">
          <el-option
...
export default {
  ...
  methods: {
    ...
    // 点击确认按钮，设置角色 
    setRoles() {
      this.$http.put(`users/${this.userInfo.id}/role`, { rid: this.rolesId }).then(res => {
        console.log(res)
